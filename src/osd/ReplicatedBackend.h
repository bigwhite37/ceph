...
private:
  ...
  struct InProgressOp : public RefCountedObject {
    ...
    void set_tolerated_uncommit_size(const OSDMapRef& osdmap, const pg_t& pgid, const size_t acting_recovery_backfill_shards_size) {
      uint8_t pool_size = osdmap->get_pg_pool_size(pgid);
      auto primary_write_size = g_conf().get_osd_pool_default_primary_write_size(pool_size);
      unsigned quorum_size = acting_recovery_backfill_shards_size / 2 + 1;
      tolerated_uncommit_size -= std::max(primary_write_size, quorum_size);
    }
  private:
